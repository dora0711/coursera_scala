...
object Anagrams {
  ...
  /** Converts the word into its character occurrence list.
    ...
    */
  def wordOccurrences(w: Word): Occurrences = (w.toLowerCase groupBy (c => c) map (p => (p._1, p._2.size)) toList) sorted
...
//  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences =
    x map (
      occ =>
        if (y.map(_._1).contains(occ._1))
          (occ._1, occ._2 - y.find(_._1 == occ._1).get._2)
        else occ
      ) filter (_._2 > 0)
